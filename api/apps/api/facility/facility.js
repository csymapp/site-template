...
class facility extends csystem {
	...
	deleteFacilityUserByFacilityUserId = async (req, res, next) => {
		return new Promise(async (resolve, reject) => {
			console.log("--------------------|||||||||||-------------")
			resolve()
			/**
			 * only check if we are SYS_ADMIN, or ORG_ADMIN or FACILITY_ADMIN
			 */
			// let authenticatedUserId = req.authenticatedUser.userId;
			let { facilityUserId } = req.query;
			// let associated = {
			// 	include:
			// 		[
			// 			{
			...
			await to(this.sequelize.models.facilityUser.destroy({ where: { facilityUserId} }))
			resolve()
		})
	}
	...
	functionsMap = () => {
		return {
			...
			"api/facility/user?facilityUserId": {
				'DELETE': {
					func: this.deleteFacilityUserByFacilityUserId,
					requiresLogin: true,
					requiresAdmin: false,
					doForAnother: true,
					doForSelf: false,
